/**
 * Netlify Serverless Function — Tourism Data Proxy
 * Fetches Taiwan government open data CSV, processes it, returns JSON.
 * Endpoint: /.netlify/functions/tourism
 */
  ...
  return new Promise((resolve, reject) => {
    const req = https.get(url, {
      ...
      res.on('end',  () => {
        ...
        resolve(buf.toString('utf8', start));
      });
      res.on('error', reject);
    });
...
// Remove bilingual suffix e.g. "高雄市\nKaohsiung City" → "高雄市"
...
function aggregate(rows) {
  ...
  // ── year trend (real historical totals from full dataset) ──
  const yearMap = {};
  for (const r of rows) {
